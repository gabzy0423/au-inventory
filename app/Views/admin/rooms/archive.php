<?php $title = 'Archived Rooms' ?>

<div class="space-y-8" x-data="{
    search: '',
    rooms: <?= htmlspecialchars(json_encode($rooms), ENT_QUOTES, 'UTF-8') ?>,
    currentPage: 1,
    itemsPerPage: 8,
    get filteredRooms() {
    ...
    get totalPages() {
        return Math.ceil(this.filteredRooms.length / this.itemsPerPage) || 1;
    },
    get paginatedRooms() {
    ...
    <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-6">
        <div>
            <div class="flex items-center gap-2 text-[10px] font-black uppercase tracking-widest text-slate-400 mb-2">
                <a href="<?= $base_url ?>/admin/rooms" class="hover:text-indigo-600 transition-colors">Rooms</a>
                <svg class="w-3 h-3" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2.5" d="M9 5l7 7-7 7" />
                </svg>
                <span class="text-slate-300">Archive Vault</span>
            </div>
            <h1 class="text-3xl font-black text-slate-900 tracking-tight italic">Archived Rooms</h1>
            <p class="text-slate-500 font-medium mt-1">Review and restore physical spaces that have been retired from active use.</p>
        </div>
    </div>
    ...
    <div class="bg-white p-4 rounded-2xl border border-slate-100 shadow-sm flex flex-col md:flex-row gap-4 items-center">
        <div class="relative flex-1 w-full">
            ...
            <input type="text" x-model="search" placeholder="Search by room name..."
    ...
    <div class="bg-white rounded-[2rem] shadow-sm border border-slate-100 overflow-hidden">
        <div class="overflow-x-auto">
            <table class="w-full text-left border-collapse">
                <thead class="bg-slate-50/50">
                    <tr>
                        <th class="px-8 py-6 text-[10px] font-black uppercase text-slate-400 tracking-widest">Room Details</th>
                        <th class="px-6 py-6 text-[10px] font-black uppercase text-slate-400 tracking-widest text-center">Identity Color</th>
                        <th class="px-6 py-6 text-[10px] font-black uppercase text-slate-400 tracking-widest text-center">Linked Assets</th>
                ...
                <tbody class="divide-y divide-slate-50">
                    <template x-for="room in paginatedRooms" :key="room.id">
                        <tr class="hover:bg-slate-50/50 transition-colors group">
                            <td class="px-8 py-6">
                                <span class="font-bold text-slate-800 block text-sm" x-text="room.name"></span>
                                <span class="text-[10px] text-slate-400 uppercase tracking-widest font-black">Archived facility</span>
                            </td>
                            <td class="px-6 py-6 text-center">
                                <div class="flex items-center justify-center">
                                    <div :class="'w-6 h-6 rounded-lg bg-' + (room.color || 'slate') + '-500 shadow-sm border-2 border-white ring-1 ring-slate-200'"></div>
                                </div>
                            </td>
                            <td class="px-6 py-6 text-center">
                                <span class="px-3 py-1 bg-slate-100 text-slate-600 text-[10px] font-black rounded-lg uppercase tracking-widest border border-slate-200/50" x-text="room.item_count + ' items'"></span>
                            </td>
                            <td class="px-8 py-6 text-right">
                                <form :action="'<?= $base_url ?>/admin/rooms/restore/' + room.id" method="POST" class="inline">
                                    <button type="submit"
        ...
        <div class="p-8 bg-slate-50/30 border-t border-slate-50 flex items-center justify-between">
            <span class="text-[10px] font-black uppercase tracking-widest text-slate-400"
                x-text="'Showing ' + ((currentPage - 1) * itemsPerPage + (paginatedRooms.length > 0 ? 1 : 0)) + ' to ' + ((currentPage - 1) * itemsPerPage + paginatedRooms.length) + ' of ' + filteredRooms.length + ' entries'"></span>
