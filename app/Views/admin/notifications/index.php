...
<div class="max-w-4xl mx-auto space-y-8" x-data="{ 
    ...
    <div class="flex flex-col md:flex-row md:items-center justify-between gap-6">
        <div>
            <h1 class="text-4xl font-black text-slate-900 tracking-tight italic">Admin Notifications</h1>
            <p class="text-slate-500 font-medium mt-1">Real-time alerts and system activity logs.</p>
        </div>
    ...
    <div class="bg-white rounded-[3rem] border border-slate-100 shadow-sm overflow-hidden flex flex-col">
        <?php if (empty($notifications)): ?>
            <div class="p-20 flex flex-col items-center justify-center text-center">
                ...
                <h3 class="text-xl font-black text-slate-900 mb-2 italic">Clean Slate</h3>
                <p class="text-sm font-medium text-slate-500 max-w-xs mx-auto">No unread alerts in your administration queue.</p>
            </div>
        <?php else: ?>
            <div class="divide-y divide-slate-50 flex-1">
                <?php foreach ($notifications as $index => $notification): ?>
                    <div x-show="Math.floor(<?= $index ?> / perPage) === currentPage - 1" 
                         class="p-8 hover:bg-slate-50/50 transition-all group relative <?= !$notification['is_read'] ? 'bg-indigo-50/20' : '' ?>">
                        <div class="flex items-start gap-6">
                            <!-- Icon / Indicator -->
                            <div class="relative">
                                <div class="w-14 h-14 rounded-2xl flex items-center justify-center transition-all group-hover:scale-110 <?= !$notification['is_read'] ? 'bg-indigo-100 text-indigo-600 shadow-lg shadow-indigo-200/50' : 'bg-slate-100 text-slate-400' ?>">
                            ...
                            <div class="flex-1">
                                <div class="flex items-center justify-between mb-2">
                                    <h4 class="text-lg font-black text-slate-900 group-hover:text-indigo-600 transition-colors italic leading-tight">
                                ...
                                <?php if (!$notification['is_read']): ?>
                                    <form action="<?= $base_url ?>/admin/notifications/read/<?= $notification['id'] ?>" method="POST" class="mt-4">
                                        <button type="submit" class="text-[10px] font-black uppercase tracking-[0.2em] text-indigo-600 hover:text-indigo-700 transition-all flex items-center gap-2">
                                            Mark as read
