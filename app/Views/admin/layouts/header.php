<header class="glass-header h-20 px-8 sticky top-0 z-20" x-data="{ showLogoutModal: false }">
    <div class="max-w-7xl mx-auto h-full flex items-center justify-between">
        <div class="flex items-center gap-4">
            ...
            <h2 class="text-xl font-bold text-slate-800 hidden sm:block">AU Inventory Portal</h2>
            <span class="px-3 py-1 bg-indigo-100 text-indigo-700 text-xs font-bold rounded-full uppercase tracking-wider">Admin</span>
        </div>
        ...
        <div class="flex items-center gap-6">
            ...
            ?>
            <a href="<?= $base_url ?>/admin/notifications" class="relative group p-2.5 bg-slate-50 border border-slate-100 rounded-2xl text-slate-400 hover:text-indigo-600 hover:bg-indigo-50 hover:border-indigo-100 transition-all">
                <svg class="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2.5" d="M15 17h5l-1.405-1.405A2.032 2.032 0 0118 14.158V11a6.002 6.002 0 00-4-5.659V5a2 2 0 10-4 0v.341C7.67 6.165 6 8.388 6 11v3.159c0 .538-.214 1.055-.595 1.436L4 17h5m6 0v1a3 3 0 11-6 0v-1m6 0H9" />
                </svg>
                <?php if ($unreadCount > 0): ?>
                    <span class="absolute top-0 right-0 -mr-1 -mt-1 w-5 h-5 bg-indigo-600 text-white text-[10px] font-black rounded-full flex items-center justify-center border-2 border-white shadow-sm animate-pulse">
                        <?= $unreadCount > 9 ? '9+' : $unreadCount ?>
                    </span>
                <?php endif; ?>
            </a>

            <a href="<?= $base_url ?>/admin/profile" class="flex items-center gap-4 group hover:bg-slate-50/80 p-1.5 pr-4 rounded-2xl transition-all">
                <div class="hidden md:flex flex-col items-end">
                    <span class="text-sm font-bold text-slate-800 group-hover:text-indigo-600 transition-colors"><?= $_SESSION['name'] ?? 'Admin User' ?></span>
                    <span class="text-[9px] font-black text-slate-400 uppercase tracking-widest leading-none mt-1"><?= strtoupper($_SESSION['role'] ?? 'System Administrator') ?></span>
                </div>
                <div class="h-10 w-10 rounded-full bg-gradient-to-tr from-indigo-500 to-purple-500 flex items-center justify-center text-white font-black shadow-lg shadow-indigo-100 group-hover:scale-110 transition-transform overflow-hidden">
