    ...
    <div class="mb-8 flex flex-col md:flex-row md:items-center justify-between gap-6">
        <div>
            <div class="flex items-center gap-2 text-[10px] font-black uppercase tracking-widest text-slate-400 mb-2">
                ...
                <span class="text-slate-300">Asset Details</span>
            </div>
            <h1 class="text-4xl font-black text-slate-900 tracking-tight italic"><?= htmlspecialchars($item['name']) ?></h1>
            <p class="text-slate-500 font-medium mt-1">Viewing full specifications and tracking data for this node.</p>
        </div>
        ...
        <div class="flex items-center gap-3">
            ...
            <a href="<?= $base_url ?>/admin/report/create/<?= $item['id'] ?>"
    ...
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-8">
        ...
        <div class="lg:col-span-2 space-y-8">
            ...
            <div class="bg-white p-8 md:p-10 rounded-[3rem] shadow-sm border border-slate-100 relative overflow-hidden group">
                ...
                <div class="space-y-8">
                    <div class="space-y-3">
                        <label class="text-[10px] font-bold text-slate-400 uppercase tracking-widest px-1">Display Name</label>
                        <div class="w-full px-6 py-5 bg-slate-50/50 border-2 border-slate-100 rounded-2xl font-bold text-slate-700 text-xl italic">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
                        <div class="space-y-3">
                            ...
                            <div class="w-full px-6 py-5 bg-slate-50/50 border-2 border-slate-100 rounded-2xl font-bold text-slate-700">
                                <?= htmlspecialchars($item['asset_number'] ?? 'N/A') ?>
                            </div>
                        </div>
                        <div class="space-y-3">
                            <label class="text-[10px] font-bold text-slate-400 uppercase tracking-widest px-1">Model / Brand</label>
                            <div class="w-full px-6 py-5 bg-slate-50/50 border-2 border-slate-100 rounded-2xl font-bold text-slate-700">
                                <?= htmlspecialchars($item['model'] ?? 'N/A') ?>
                            </div>
                        </div>
                        <div class="space-y-3">
                            <label class="text-[10px] font-bold text-slate-400 uppercase tracking-widest px-1">Serial Number</label>
                            <div class="w-full px-6 py-5 bg-slate-50/50 border-2 border-slate-100 rounded-2xl font-bold text-slate-700">
                    ...
                    <div class="space-y-3">
                        ...
                        <div class="w-full px-6 py-5 bg-slate-50/50 border-2 border-slate-100 rounded-2xl font-medium text-slate-600 leading-relaxed">
            ...
            <div class="bg-white p-8 md:p-10 rounded-[3rem] shadow-sm border border-slate-100">
                ...
                <div class="space-y-4">
                    <?php if (empty($reports)): ?>
                        <div class="p-10 border-2 border-dashed border-slate-100 rounded-[2.5rem] flex flex-col items-center justify-center text-center">
                            <div class="w-16 h-16 bg-emerald-50 text-emerald-500 rounded-2xl flex items-center justify-center mb-4">
                                <svg class="w-8 h-8" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 12l2 2 4-4m6 2a9 9 0 11-18 0 9 9 0 0118 0z" /></svg>
                            </div>
                            <p class="text-sm font-bold text-slate-800">Operational Health: Optimal</p>
                            <p class="text-xs text-slate-400 mt-1 font-medium">No active maintenance reports found for this node.</p>
                        </div>
                    <?php else: ?>
                        <?php foreach ($reports as $report): ?>
                            <div class="p-6 bg-slate-50 border-2 border-slate-100 rounded-3xl flex flex-col md:flex-row md:items-center justify-between gap-4 group hover:border-indigo-200 transition-all">
                                <div class="flex items-start gap-4">
                                    ...
                                    <div>
                                        <div class="flex items-center gap-2">
                                            <p class="text-sm font-black text-slate-800 italic"><?= htmlspecialchars($report['title']) ?></p>
                                        ...
                                        <p class="text-xs text-slate-500 font-medium line-clamp-1"><?= htmlspecialchars($report['description']) ?></p>
                                        <p class="text-[10px] text-slate-400 font-bold uppercase mt-1"><?= date('M d, Y', strtotime($report['reported_at'])) ?></p>
                                    </div>
                                </div>
                                <?php 
                                    $rStyles = [
                                ...
                                ?>
                                <span class="px-4 py-1.5 <?= $style ?> text-[10px] font-black rounded-full uppercase tracking-widest self-start md:self-center">
                                    <?= $report['status'] ?>
                                </span>
                            </div>
                        <?php endforeach; ?>
            ...
            <div class="bg-white p-8 md:p-10 rounded-[3rem] shadow-sm border border-slate-100">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-8">
                    <div class="space-y-3">
                        <label class="text-[10px] font-bold text-slate-400 uppercase tracking-widest px-1">Date of Deployment</label>
                        <div class="w-full px-6 py-5 bg-slate-50/50 border-2 border-slate-100 rounded-2xl font-bold text-slate-700">
                            <?= $item['deployment_date'] ? date('M d, Y', strtotime($item['deployment_date'])) : 'N/A' ?>
                        </div>
                    </div>
                    <div class="space-y-3">
                        <label class="text-[10px] font-bold text-slate-400 uppercase tracking-widest px-1">Registered At</label>
                        <div class="w-full px-6 py-5 bg-slate-50/50 border-2 border-slate-100 rounded-2xl font-bold text-slate-700">
        ...
        <div class="space-y-8">
            ...
            <div class="bg-white p-8 rounded-[3rem] shadow-sm border border-slate-100 flex flex-col items-center">
                <h3 class="text-[10px] font-black uppercase tracking-[0.2em] text-indigo-500 mb-8 self-start flex items-center gap-3">
                    <span class="w-8 h-[2px] bg-indigo-500 rounded-full"></span>
                    Tracking Tag
                </h3>

                <div class="p-6 bg-slate-50 rounded-[2.5rem] border-2 border-slate-100 mb-6 group transition-all hover:bg-white hover:shadow-xl hover:shadow-indigo-500/10 tracking-tag-qr">
                ...
                <div class="text-center mb-6">
                    <span class="text-2xl font-black text-slate-900 tracking-widest block uppercase"><?= htmlspecialchars($item['tag']) ?></span>
                    <p class="text-[10px] font-bold text-slate-400 uppercase tracking-widest mt-2">Unique Identification Tag</p>
                </div>

                <button onclick="downloadQR()" 
                    class="w-full py-4 bg-slate-900 text-white rounded-2xl font-black text-xs uppercase tracking-widest hover:bg-slate-800 transition-all flex items-center justify-center gap-2 shadow-xl shadow-slate-900/10 active:scale-95">
                    <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                ...
                <script>
                    function downloadQR() {
                        const svgElement = document.querySelector('.tracking-tag-qr svg');
                        ...
                        link.href = url;
                        link.download = 'QR_<?= htmlspecialchars($item['tag']) ?>.svg';
